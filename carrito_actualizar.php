...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "search")) {
  $updateSQL = sprintf("UPDATE tblcarrito SET idProducto=%s WHERE intContador=%s",
                       GetSQLValueString($_POST['keywords'], "int"),
                       GetSQLValueString($_POST['keywords'], "int"));

  mysql_select_db($database_conexionzapatos, $conexionzapatos);
  $Result1 = mysql_query($updateSQL, $conexionzapatos) or die(mysql_error());

  $updateGoTo = "carrito_lista.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<table border="0" width="" cellspacing="0" cellpadding="0"> 
   <tr> 
	 ...
	 <!-- InstanceBeginEditable name="titulo" -->ACTUALIZANDO  PRODUCTO DE LA CESTA<!-- InstanceEndEditable --> <br/>
	 <!-- InstanceBeginEditable name="contenido" -->Actualizando...<!-- InstanceEndEditable --><br> 
       <br></td>
   </tr> 	
</table> 
