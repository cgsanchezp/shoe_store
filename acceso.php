...
// Se guarda en una variable los datos obtenidos
  $loginUsername=$_POST['strEmail'];
  $password=$_POST['strPassword'];
  $MM_fldUserAuthorization = "";
  $CodigoCorrecto = "acceso_ok.php";
  $CodigoIncorrecto = "acceso_error.php";
  $MM_redirecttoReferrer = false;
  mysql_select_db($database_conexionzapatos, $conexionzapatos);
  // Realiza el query de la tabla tblusuario
  ...
  if ($loginFoundUser) {
    ...
    //declare las variables de session
    $_SESSION['MM_Username'] = $loginUsername;
    $_SESSION['MM_UserGroup'] = $loginStrGroup;	
    $_SESSION['MM_IdUsuario'] = $row_LoginRS["idUsuario"];	

    if (isset($_SESSION['PrevUrl']) && false) {
      $CodigoCorrecto = $_SESSION['PrevUrl'];	
    }
	// Redirecciona a la pagina principal si el login es correcto
    header("Location: " . $CodigoCorrecto );
  }
  else {
  // Redirecciona a la pagina incorrecta.
    header("Location: ". $CodigoIncorrecto );
  }
}
?>
...
<table border="0" width="" cellspacing="0" cellpadding="0"> 
  ...
  <tr>
    <td>Email:<label for="strEmail"></label></td>
    <td><span id="sprytextfield1">
                    <input type="text" name="strEmail" id="strEmail" />
                    <span class="textfieldRequiredMsg">*</span><span class="textfieldInvalidFormatMsg">*</span></span></td>
  </tr>
  <tr>
    <td>Contrase&ntilde;a:<label for="strPassword"></label></td>
